...
import org.corpus-tools.pepperModules-sgsTEIModules.sgsTEIExporter;
 ...
 * {@link sgsTEIExporter} class. Feel free to adapt and enhance this test class
 ...
 * @author Martin Klotz
 */
public class sgsTEIExporterTest extends PepperExporterTest {
	/**
	...
	public void test_DummyImplementation() {
		...
		getFixture().setCorpusDesc(new CorpusDesc().setCorpusPath(URI.createFileURI(getTempPath("sgsTEIExporter").getAbsolutePath())));

		// starts the Pepper framework and the conversion process
		start();

		File superCorpus = new File(getTempPath("sgsTEIExporter").getAbsolutePath() + "/rootCorpus");
